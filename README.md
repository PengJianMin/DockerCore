...
5. mount namespace：通过隔离文件系统**挂载点**对隔离文件系统提供支持，隔离的是使用**mount命令**的效果，不同mount namespace中的文件结构发生变化互不影响
    ...
    + 挂载传播（mount propagation）：定义了挂载对象（mount object）之间的关系，这样的关系包括共享关系和从属关系，系统用这些关系决定任何挂载对象中的挂载事件**如何传播到其他挂载对象**
